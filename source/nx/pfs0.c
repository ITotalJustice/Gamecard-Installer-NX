...
void pfs0_get_header(pfs0_struct_ptr *ptr, uint64_t offset)
{
    ...
    read_file(&ptr->header, sizeof(pfs0_header_t), offset, ptr->file);
}
...
void pfs0_populate_file_table(pfs0_struct_ptr *ptr)
{
    ...
    read_file(ptr->file_table, ptr->file_table_size, ptr->file_table_offset, ptr->file);
}
...
void pfs0_populate_string_table(pfs0_struct_ptr *ptr)
{
    ...
    read_file(data_temp, ptr->header.string_table_size, ptr->string_table_offset, ptr->file);
...
bool pfs0_extract_file(pfs0_struct_ptr *ptr, int location)
{
    ...
    if (!new_file)
    {
        write_log("failed to create new file %s\n", ptr->string_table[location].name);
        return false;
    }
    ...
    for (size_t offset = 0, buf_size = 0x800000; offset < file_size; offset += buf_size)
    {
        ...
        read_file(buf, buf_size, curr_off + offset, ptr->file);
...
bool pfs0_process(pfs0_struct_ptr *ptr, uint64_t offset, FILE *fp)
{
    if (!ptr || !fp)
    {
        write_log("missing params in pfs0_process\n");
        return false;
    }
